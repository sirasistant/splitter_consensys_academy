...
contract('Splitter', function (accounts) {
  ...
  it("should register correctly the receivers", () => {
    ...
    return instance.register(receivers[0],receivers[1],{from:accounts[1]}).then(()=>{
      ...
      .then(split=>{
        assert.equal(split[0][0],receivers[0]);
        assert.equal(split[0][1],receivers[1]);
      })
    })
  });
  ...
  it("should split the balance sent", () => {
    ...
    return Promise.all(accountsToSplit.map((account) => instance.balanceOf.call(accounts[0],account)))
      .then((_balances) => {
        balances = _balances;
        return instance.split({ from: accounts[0], value: amountToSend });
      }).then(() => Promise.all(accountsToSplit.map((account) => instance.balanceOf.call(accounts[0],account))))
      .then((newBalances) => {
        ...
        return Promise.all(accountsToSplit.map((account)=>instance.withdraw(accounts[0],{from:account})))
      }).then(()=>{
        return Promise.all(accountsToSplit.map((account) => instance.balanceOf.call(accounts[0],account)))
      }).then((balances)=>{
